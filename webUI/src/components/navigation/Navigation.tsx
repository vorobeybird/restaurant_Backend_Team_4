    ...
    <div className="navigation">
      ...
      <div className="links">
        {links.map(({ title, link }) => {
          return (
            <Link key={title}
              className={location && location.pathname === link
                ? "link link__active" : "link"}
              to={link}>
              {title}
            </Link>
          );
      ...
      <div className="input_container">
        ...
        <img className={location && location.pathname.includes("/search")
          ? "search_icon search_icon_active" : "search_icon"}
          src={SearchIcon} alt="search icon"
          onClick={() => changeRoute('/search')}></img>
      </div>
      <div className="navigation_icons_container">
        ...
        <Link to={user && user.username.length > 0 ? "/profile" : "/login"} onClick={() => { }}>
          <img
            className={location && location.pathname.includes("/profile")
              ? "profile_icon profile_icon_active" : "profile_icon"}
            src={Profile}
            alt="profile icon"
          ></img>
        </Link>
        ...
        <ul className="menu__box">
          <div className="menu__item">
            ...
            <Link to={user && user.username.length > 0 ? "/cart" : "/"}>
              <img className={location && location.pathname === "/cart"
                ? "cart_icon cart_icon_active" : "cart_icon"}
                src={Cart} alt="cart icon"></img>
            </Link>
            <Link to="/profile">
              <img className={location && location.pathname.includes("/profile")
                ? "profile_icon profile_icon_active" : "profile_icon"}
                src={Profile} alt="profile icon"></img>
            </Link>
          </div>
          {links.map(({ title, link }) => {
            return (
              <Link
                key={title}
                className={location && location.pathname === link 
                  ? "menu__item menu__item_active" : "menu__item"}
                to={link}>
                {title}
              </Link>
            );

...
export const Search = () => {
  ...
  const makeSearch = () => {
    const filtredDishes = items.filter((el) =>
      el.title.toLowerCase().startsWith(searchString.toLowerCase())
    );
  ...
  return (
    <div className="search">
      <div className="search__container">
        ...
        <div
          className={
            !searchString.length ? "search__result" : "search__result_active"
          }
        >
          {!searchString.length ? (
              ...
              {!makeSearch().length ? (
                ...
                makeSearch().map((el, index) => (
                  <Link to="/dishPage" onClick={() => handleDishClick(el)}>
                    {el.photo.length > 0 ? (
                      <img
                        src={el.photo[0].photo_url}
                        alt="dish-main-photo"
                      ></img>
                    ) : null}
                    <div className="search__dish-title">{el.title}</div>
                    <div className="search__dish-price">
                      <span>{el.price}</span>&#8194;BYN
                    </div>
                  </Link>
                ))
              )}

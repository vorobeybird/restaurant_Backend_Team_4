...
export const ImageSlider: FunctionComponent<IimageSliderProps> = (props) => {
    ...
    return (
        <div className="slider">
            <button className="slider-left-arrow" onClick={prevSlide} disabled={current === 0 ? true : false}>
            ...
            {props.sliderData.map((slide: any, index: number) => {
                return (
                    <div className={index === current ? "slide active" : "slide"} key={index}>
                        {index === current &&
                            (<LazyLoadImage src={slide.image} alt="dish-image" className="slider-img" effect="blur"/>)}
                    </div>
                )
            })}
            <button className="slider-right-arrow" onClick={nextSlide} disabled={current === length - 1 ? true : false}>
